...
// The lib self-initializes coercion from the embedded catalog so every consumer
// gets ISO-date handling without calling SetDateKeys.
func init() { SetDateKeys(catalog.DateKeys()) }
...
//   - Relative "now", "now-<N><unit>" and "-<N><unit>" (unit s/m/h/d/w) resolve
//     against the current UTC time to a past instant.
...
func MaybeDate(name, value string) (string, error) {
	...
	if ms, ok := parseRelative(value); ok {
		return strconv.FormatInt(ms, 10), nil
	}
	return "", fmt.Errorf("--%s: %q is not a valid date (expected ISO-8601, epoch-millis, or relative like now / now-7d / -24h)", name, value)
}

// parseRelative resolves "now", "now-<N><unit>" and "-<N><unit>" against the
// current UTC time. Units: s, m, h, d, w (case-insensitive). Only past-relative
// forms are accepted — no future offsets, no "ago" phrasing.
func parseRelative(value string) (int64, bool) {
	...
	return now.Add(-time.Duration(n) * unit).UnixMilli(), true
}
